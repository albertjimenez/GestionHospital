...
public class PruebaPacientes {
	...
	public void test() {
		...
		for (int i = 0; i < 20; i++) {
			int key = i;
			Paciente value = new Paciente("s", "a", key, new GregorianCalendar(
					1, 1, 1), "h", "h", "h", "h", 12500, "d");
			gestor.getMapaPacientes().put(key, value);

		}
		System.out.println(gestor.getMapaPacientes().values().toString());
		// System.out.println(gestor.getMapaPacientes().toString());
		assertEquals(20, gestor.getMapaPacientes().size(), 0);
	...
	public void testEdicion() {
		Paciente p1 = new Paciente("sa", "a", 5000, new GregorianCalendar(1, 1,
				1), "h", "h", "h", "h", 12500, "d");
		gestor.addPaciente(p1);
		// System.out.println(gestor.getMapaPacientes().toString());

		assertEquals("sa", gestor.getMapaPacientes().get(5000).getNombre());
		Paciente p2 = new Paciente("sab", "a", 5000, new GregorianCalendar(1,
				1, 1), "h", "h", "h", "h", 12500, "d");
		...
		Paciente p3 = new Paciente("sab", "a", 5200, new GregorianCalendar(1,
				1, 1), "h", "h", "h", "h", 12500, "d");
	...
	public void testBorrar() {
		Paciente p1 = new Paciente("sa", "a", 5000, new GregorianCalendar(1, 1,
				1), "h", "h", "h", "h", 12500, "d");
		Paciente p2 = new Paciente("sa", "a", 5100, new GregorianCalendar(1, 1,
				1), "h", "h", "h", "h", 12500, "d");
		gestor.addPaciente(p1);
